/**
  * @file    bsp_lcd.h
  * @brief   LCD 驱动头文件 - 支持 STM32F429 LTDC + DMA2D，双图层，动态旋转，多种像素格式
  * @author  Your Name
  * @version V1.2
  * @date    2026-06-23
  * 
  * @details 本驱动提供以下核心功能：
  *          1. 双图层管理：背景层（BG）和前景层（FG），可独立设置窗口裁剪。
  *          2. 动态横竖屏切换：支持 0°、90°、180°、270° 旋转，所有绘图坐标自动映射。
  *          3. 动态像素格式：RGB565、RGB888、ARGB8888 运行时切换，颜色值统一以 RGB565 宏输入。
  *          4. 丰富绘图原语：点、线、圆（空心/实心）、矩形（空心/实心）。
  *          5. 字符/字符串显示：基于光标位置，支持换行、回车、自动换行，背景透明/不透明可选。
  *          6. 数值与格式化输出：十进制、十六进制、浮点数、printf 风格格式化。
  *          7. 硬件加速：DMA2D 块填充，用于清屏和矩形填充，大幅提升性能。
  * 
  * @note   1. 所有坐标均为逻辑坐标（相对于当前旋转方向），内部自动映射到物理坐标。
  *         2. 颜色输入使用 RGB565 宏（如 COLOR_RED），驱动内部自动转换为当前格式。
  *         3. 需外部提供 DMA2D 句柄 hdma2d 和 LTDC 初始化（图层像素格式需匹配）。
  *         4. 物理尺寸 LCD_PHYS_WIDTH/HEIGHT 应与 LTDC 输出配置一致。
  *         5. 野火STM32F429IGT6,配套5寸显示屏注意事项，使用ARGB8888格式，LTDC的时钟频率不应高于19MHZ。
  *         6. 野火STM32F429IGT6,配套5寸显示屏注意事项，使用ARGB565格式，LTDC的时钟频率不应高于25MHZ。
  *         7. SDRAM型号为W9825G6KH-6I，不应配置为写突发，即WB位为1，仅读突发，突发长度为4。
  */
...
#include "stm32f4xx_hal.h"        /* HAL 库，提供基本类型和 DMA2D 定义 */
 ...
 *============================================================================*/
/**
  * @defgroup LCD_Text_Params 文本绘制参数（可调）
  * @{
  */
#define LCD_LINE_OFFSET          0U     /*!< 行间距（像素），用于自动换行时增加的行间隔 */
#define LCD_CHAR_SPACE           0U     /*!< 字符间距（像素），字符之间的额外间隔 */
/** @} */
 ...
 *============================================================================*/
/**
  * @defgroup LCD_Global_Vars 全局状态变量（可读写）
  * @{
  */
...
/**
  * @brief 初始化 LCD 硬件（使能 DMA2D，初始化句柄，设置默认格式/旋转，清屏）
...
void BSP_LCD_FillBackground(uint16_t color);     /*!< 填充背景层全屏 */
void BSP_LCD_FillForeground(uint16_t color);     /*!< 填充前景层全屏 */
